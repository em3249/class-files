...
{
    arp_packet_t *pack = (arp_packet_t *)raw_packet;

    pack->arp_hdr.htype = ntohs(pack->arp_hdr.htype);
...
icmp_echo_packet_t *process_icmp_echo(icmp_packet_t *icmp)
{
    ...
    pack->icmp_echo_hdr.id = ntohs(pack->icmp_echo_hdr.id);
    pack->icmp_echo_hdr.sequence = ntohs(pack->icmp_echo_hdr.sequence);

    pack->icmp_echo_hdr.timestamp = ntohl(pack->icmp_echo_hdr.timestamp);
    pack->icmp_echo_hdr.timestamp_ms = ntohl(pack->icmp_echo_hdr.timestamp_ms);
