...
// This function establishes a connection to the specified host and port.
// This function resolves the hostname to an IP address using `gethostbyname`.
// Then copies the resolved address to a `sockaddr_in` structure. Next, 
// it creates a socket using the `socket` function. Finally, it 
// attempts to connect to the server using the `connect` function.
// If any step fails, the function returns a negative value and prints an error message.
...
// This functino calculates the length of the HTTP header in the given buffer.
// It searches for the end of the HTTP header, indicated by a specific sequence of characters ("\r\n\r\n").
// It uses the `strnstr` function to locate the end of the header and calculates the header length based on the position of the header end.
// If the end of the header is not found, it prints an error message and returns a negative number.
...
// This function extracts the Content-Length value from the HTTP header in the provided buffer.
// It parses the HTTP header line by line to find the Content-Length header.
// It uses `sscanf` to read each header line and `strcasestr` to search for the Content-Length header.
// If the Content-Length header is found, it extracts and returns the Content-Length value.
// If the header is not found, it prints an error message and returns 0.
